...
The code I am posting here is a blended and modified version of the code shared by Pfeiffer and Lukito. It is inspired by the code shared by Lukito, in that it creates an R loop to apply the test to a multivariate VAR (it is therefore appliable both to bivariate and multivariate VARs), and uses the approach and functions suggested by Pfeiffer. Moreover, the function can be applied to any VAR model and makes it easier and faster to run the analysis.
